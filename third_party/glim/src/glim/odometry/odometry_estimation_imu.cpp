...
OdometryEstimationIMUParams::OdometryEstimationIMUParams() {
  ...
  imu_acc_noise = sensor_config.param<double>("sensors", "imu_acc_noise", 0.01);
  imu_gyro_noise = sensor_config.param<double>("sensors", "imu_gyro_noise", 0.001);
  imu_int_noise = sensor_config.param<double>("sensors", "imu_int_noise", 0.001);
...
OdometryEstimationIMU::OdometryEstimationIMU(std::unique_ptr<OdometryEstimationIMUParams>&& params_) : params(std::move(params_)) {
  ...
  IMUIntegrationParams imu_integration_params;
  imu_integration_params.acc_noise = params->imu_acc_noise;
  imu_integration_params.gyro_noise = params->imu_gyro_noise;
  imu_integration_params.int_noise = params->imu_int_noise;
  imu_integration.reset(new IMUIntegration(imu_integration_params));
